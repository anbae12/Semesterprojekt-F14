...
\paragraph{Metode 3\\}
\todo[inline,author=Mikael]{Indsæt tabel med induktans for forskellige motorpositioner og forskellige testfrekvenser}
I tabel \ref{tb:lcr} findes induktansmålingerne fra LCR-meteret.
\begin{figure}[th!]
	\centering
	\input{content/tb_lcr.tex}
	\captionsetup{type=table}
...
Motorens induktans estimeres derfor til at være ca. 3 [mH]\todo{3 mH???}.
...
Til modelleringen af DC motoren estimeres motorens induktans til \(L_m=3\) \([mH]\).
...
\item \(L_m = 3,047\) \(\text{[mH]}\) \todo{update value}
